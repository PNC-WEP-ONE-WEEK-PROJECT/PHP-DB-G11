...
function uploadImage($photo)
{
    ...
    if ($error === 0) {
        ...
        $img_ex_lc = strtolower($img_ex); //Convert all characters to lowercase:
...
function createPost($description, $photo)
{
    global $db;
    $statement = $db->prepare("INSERT INTO posts(postDate, description, image,  user_ID) values (now(), :description, :image , 2);");
    $statement->execute([
